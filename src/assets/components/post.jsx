...
function Post({ data, opened, setOpened, small }) {
  ...
  return (
    <div
      ...
      className={`text-white rounded-3xl shadow-lg  mas-h-fit  transition-all overflow-hidden ${small ? "grid grid-cols-4" : ""} ${opened ? "col-start-1 bg-gray-800 max-h-fit row-start-1  top-0 bottom-0 md:col-span-1 lg:col-span-1 h-full" : "cursor-pointer mask-b-from-85% bg-gray-800 border-gray-700 border-1 "} ${small ? "max-h-fit" : ""}`}
    >
      <img
        src={thumbnail}
        alt={name}
        className={`w-full p-2 rounded-3xl object-cover ${small ? " m-auto rounded-2xl cols-span-1" : ""}`}
      />
      <div className={`p-4 ${small ? "col-span-3" : ""}`}>
        <h2 className="text-3xl font-bold text-violet-400 mb-2">{name}</h2>

        <p className={`text-gray-400 mb-4 ${!opened && "line-clamp-2"}`}>
          {description}
        </p>

        <div className="flex flex-wrap gap-2 mb-4">
          {tags?.map((i) => (
            <p key={i} className="bg-blue-600 px-3 py-1 rounded-full text-sm">
              {i}
            </p>
          ))}
        </div>
        <div className={` ${opened ? "block" : "hidden"}`}>
          <div className="grid grid-cols-2 gap-4 text-gray-300">
            <p>
              <span className="font-bold text-violet-300">Price:</span> {price}
            </p>
            <p>
              <span className="font-bold text-violet-300">Rating:</span>{" "}
              {"⭐".repeat(rating)}
            </p>
            <p>
              <span className="font-bold text-violet-300">Producer:</span>{" "}
              {producer}
            </p>
            <p>
              <span className="font-bold text-violet-300">Price Model:</span>{" "}
              {priceModel}
            </p>
            <p>
              <span className="font-bold text-violet-300">Category:</span>{" "}
              {category}
            </p>
            <p>
              <span className="font-bold text-violet-300">SubCategory:</span>{" "}
              {subCategory}
            </p>
            <p>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {integrations?.map((i) => (
                  <p key={i} className="bg-gray-700 px-2 py-1 rounded">
                    {i}
                  </p>
                ))}
              </div>
